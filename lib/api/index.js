...
const getAllValidators = async () => {
  ...
  if (process.env.NEXT_PUBLIC_USE_MOCK_DATA && process.env.NEXT_PUBLIC_USE_MOCK_DATA === 'true') {
    ...
    validators = await api.post(`https://api.avax.network/ext/bc/P?method=platform.getCurrentValidators`, payload, {
      cache: {
...
const getLastBlockHeight = async () => {
  ...
  if (process.env.NEXT_PUBLIC_USE_MOCK_DATA && process.env.NEXT_PUBLIC_USE_MOCK_DATA === 'true') {
    ...
    result = await api.post(`https://api.avax.network/ext/bc/P?method=platform.getHeight`, payload, {
      cache: {
...
const getCurrentSupply = async () => {
  ...
  if (process.env.NEXT_PUBLIC_USE_MOCK_DATA && process.env.NEXT_PUBLIC_USE_MOCK_DATA === 'true') {
    ...
    result = await api.post(`https://api.avax.network/ext/bc/P?method=platform.getCurrentSupply`, payload, {
      cache: {
...
const getTotalStake = async () => {
  ...
  if (process.env.NEXT_PUBLIC_USE_MOCK_DATA && process.env.NEXT_PUBLIC_USE_MOCK_DATA === 'true') {
    ...
    result = await api.post(`https://api.avax.network/ext/bc/P?method=platform.getTotalStake`, payload, {
      cache: {
